...
export interface DateFieldRangePickerProps
	extends ComponentPropsWithRef<typeof DateRangePicker> {
	label?: string;
	error?: string;
}

export function DateFieldRangePicker({
	label,
	error,
	...props
}: DateFieldRangePickerProps) {
	return (
		<DateRangePicker className={cn('*:not-first:mt-2', props.className)} {...props}>
			<Label className="text-sm font-medium text-foreground">{label}</Label>
			<div className="flex">
				<Group className={cn(dateInputStyle, 'pe-9', error && 'border-red-500')}>
