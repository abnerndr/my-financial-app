...
export interface MultipleFilesUploaderTableProps {
	maxFiles?: number;
	initialFiles?: InitialFileType[]; // Initial files to display
	onGetFiles?: (files: File[]) => void;
}

export function MultipleFilesUploaderTable({
	maxFiles = 10,
	initialFiles,
	onGetFiles,
			...
			>
				...
				<div className="flex flex-col items-center justify-center text-center">
					...
					<p className="text-xs text-muted-foreground">
						Até {maxFiles} arquivos ∙ Até {formatBytes(maxSize)}
					</p>
						...
						<Table>
							<TableHeader className="text-xs">
								<TableRow className="bg-muted/50">
									...
									<TableHead className="h-9 w-0 py-2 text-right">Ações</TableHead>
								</TableRow>
							</TableHeader>
